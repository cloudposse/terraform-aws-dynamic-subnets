...
| <a name="input_nat_instance_cpu_credits_override"></a> [nat\_instance\_cpu\_credits\_override](#input\_nat\_instance\_cpu\_credits\_override) | NAT Instance credit option for CPU usage. Valid values include standard or unlimited. T3 instances are launched as unlimited by default. T2 instances are launched as standard by default. | `string` | `""` | no |
...
| <a name="input_single_nat_gateway"></a> [single\_nat\_gateway](#input\_single\_nat\_gateway) | Flag to use a single NAT Gateway across all AZs. Note that this isn't changing the private routes number, it is just using the same NGW on all of them to save money and it's therefore not recommended for production. Since it is just using the same NGW it can be updated after the deployment to switch between single and multi-AZ configuration. It defaults to false (multi-AZ) | `bool` | `false` | no |
