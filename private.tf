...
resource "aws_subnet" "private" {
  ...
  cidr_block        = "${cidrsubnet(signum(length(var.cidr_block)) == 1 ? var.cidr_block : data.aws_vpc.default.cidr_block, ceil(log(length(var.availability_zones) * 2, 2)), count.index)}"
